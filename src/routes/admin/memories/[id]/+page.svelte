<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			error = 'Failed to load album: ' + err.message;
		} finally {
			loading = false;
		}
	});
	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			await updateDoc(doc(db, 'memories', id), docData);
			goto('/admin/memories');
		} catch (err: any) {
...
{#if loading}
	<div class="flex h-[calc(100vh-64px)] items-center justify-center bg-gray-50">
